...
__all__ = ["mymuladd", "myadd_out", "elu", "elu_cuda_out"]
...
@torch.library.register_fake("hpco::mymuladd")
def _(a, b, c):
...
def _backward(ctx, grad):
    ...
    if ctx.needs_input_grad[0]:
        grad_a = torch.ops.hpco.mymul.default(grad, b)
    if ctx.needs_input_grad[1]:
        grad_b = torch.ops.hpco.mymul.default(grad, a)
...
@torch.library.register_fake("hpco::elu")
def _(a):
    torch._check(a.dtype == torch.float)
    return torch.empty_like(a)
